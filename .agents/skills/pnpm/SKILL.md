...
- Prefer named catalogs in `pnpm-workspace.yaml` over raw version ranges in `package.json`.
- When adding dependencies, use `pnpm add <pkg> --save-catalog-name <catalog>` or `pnpm add -D <pkg> --save-catalog-name <catalog>`.
- Use `catalogMode: prefer` when the workspace standard is to keep versions centralized but still allow explicit non-catalog specs when needed.
